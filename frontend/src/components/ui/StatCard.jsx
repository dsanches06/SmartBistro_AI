...
export function StatCard({ label, value, icon, valueClassName, className = "" }) {
  const { theme } = useTheme();
  const defaultValueClass = theme === "dark" ? "text-white" : "text-slate-900";
  const valueClass = valueClassName ?? defaultValueClass;

  return (
    <div className={`relative rounded-[24px] p-5 shadow-sm ${theme === "dark" ? "bg-[var(--surface)]" : "bg-white"} ${className}`}>
      {icon && (
