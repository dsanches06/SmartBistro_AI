...
function KpiCard({ faIcon, iconColor, iconBg, label, value, pct, up, sub }) {
  return (
    <div className="rounded-[20px] bg-[var(--surface)] p-4 sm:p-5 shadow-sm flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[11px] sm:text-xs font-medium truncate" style={{ color: "var(--text-muted)" }}>{label}</p>
        <p className="text-lg sm:text-2xl font-bold mt-0.5 leading-tight truncate" style={{ color: "var(--text)" }}>{value}</p>
        {pct !== null && up !== null && (
        ...
        {sub && <p className="text-[11px] sm:text-xs mt-1 truncate" style={{ color: "var(--text-muted)" }}>{sub}</p>}
      </div>
    </div>
  );
}
